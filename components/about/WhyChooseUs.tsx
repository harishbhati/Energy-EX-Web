    ...
    <section className="px-6 md:px-14 py-12 md:py-[88px] bg-off">
      <div className="max-w-[1180px] mx-auto">
        ...
        <div className="text-center mb-10 md:mb-[52px]">
          ...
          <p className="mx-auto text-[15px] md:text-[17px] text-muted leading-[1.7] max-w-[500px] font-light">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          ...
          <div className="grid gap-[14px] grid-cols-1 sm:grid-cols-2">
            {cards.map((card) => (
              <div
                key={card.title}
                className="group relative bg-white rounded-[var(--r)] overflow-hidden transition-all duration-250 cursor-default border border-[color:var(--border)] p-5 md:p-[22px]"
                onMouseEnter={(e) => {
              ...
              >
                ...
                <div className="absolute top-0 left-0 right-0 h-[3px] bg-brand-orange scale-x-0 origin-left transition-transform duration-300 group-hover:scale-x-100" />
                ...
                <h3 className="font-semibold mb-2 text-[14px] md:text-[14.5px] text-ink leading-[1.3]">
                  {card.title}
                </h3>
                <p className="text-[13px] text-muted leading-[1.65]">{card.desc}</p>
              </div>
            ))}
          </div>
          ...
          <div className="relative rounded-[var(--rl)] overflow-hidden min-h-[320px] md:min-h-[480px]">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5 md:p-7">
              ...
              >
                {bottomStats.map((s) => (
                  <div key={s.label} className="text-center py-4 px-2 md:py-5 md:px-[14px]">
                    <div className="font-serif-num text-[22px] md:text-[28px] font-bold text-white tracking-[-1px] leading-none">
                      {s.val}
                    </div>
                    <div className="mt-1.5 font-medium text-[11px] text-white/50">{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
