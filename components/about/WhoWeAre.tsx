    ...
    <section className="px-6 md:px-14 py-12 md:py-[88px]">
      <div className="max-w-[1180px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-[80px] items-center">
        ...
        <div>
          ...
          <h2 className="font-serif-num mb-[22px] text-[28px] sm:text-[36px] md:text-[46px] font-semibold text-ink tracking-[-1px] leading-[1.1]">
            Not a price comparison site.{' '}
            <em className="italic text-brand-orange">A partner.</em>
          </h2>

          <p className="mb-4 text-base text-muted leading-[1.75]">
            Energyex is an independent UK energy brokerage, founded in 2021 and built on 13 years of
            sector experience. We help businesses of all sizes — from single-site SMEs to
          ...
          <p className="mb-[40px] text-base text-muted leading-[1.75]">
            ...
            the best options for your specific business. Then we handle everything — from switching to
            billing queries to renewals.
          </p>
          ...
          <div className="flex flex-col gap-[12px]">
            {highlights.map((h) => (
              ...
              >
                <div className="w-10 h-10 rounded-[10px] flex items-center justify-center flex-shrink-0 transition-colors duration-200 group-hover:[background:var(--orange)] bg-orange-tint">
                  <h.Icon size={18} className="group-hover:text-white text-brand-orange" />
                </div>
                <div>
                  <h4 className="font-semibold mb-[3px] text-[14.5px] text-ink">{h.title}</h4>
                  <p className="text-[13.5px] text-muted leading-[1.6]">{h.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="relative">
          {/* Badge — inside image on mobile, floats outside on md+ */}
          <div
            className="absolute z-[3] rounded-[var(--r)] text-center bg-navy shadow-[0_8px_32px_rgba(13,27,42,0.28)] top-3 left-3 md:-top-[18px] md:-left-[18px]"
            style={{ padding: '14px 20px' }}
          >
            <div className="font-serif-num text-[28px] md:text-[34px] font-bold text-brand-orange-soft leading-none tracking-[-1px]">
              13+
            </div>
