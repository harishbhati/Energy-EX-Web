...
export default function HeroSection({
  ...
  return (
    <div className="relative overflow-hidden px-6 md:px-14 py-10 md:py-14 bg-navy">
      {/* Background image */}
      <div
      ...
      <div className="relative z-10 max-w-[1180px] mx-auto text-center">
        {/* Pill badge */}
        <div
          className="inline-flex items-center gap-2 rounded-[40px] mb-[22px] md:mb-[26px] px-4 py-[6px] text-[11px] font-semibold uppercase tracking-[1.5px]"
          style={{
        ...
        >
          <span className="w-[6px] h-[6px] rounded-full flex-shrink-0 animate-pulse-ring bg-brand-orange" />
          {eyebrow}
        </div>

        {/* H1 */}
        <h1 className="text-white mb-5 mx-auto max-w-[880px] text-[34px] sm:text-[46px] md:text-[62px] font-semibold leading-[1.08] tracking-[-1px]">
          {titlePart1}{' '}
          <em className="italic text-brand-orange-soft">{titlePart2}</em>
        </h1>

        {/* Subtitle */}
        <p className="mx-auto mb-8 md:mb-9 max-w-[560px] text-[15px] md:text-[17px] text-white/60 leading-[1.75] font-light">
          {subtitleStart}{' '}
          <strong className="text-white/[0.85] font-medium">{subtitleBold}</strong>{' '}
          {subtitleEnd}
        </p>
          ...
          {stats.map((stat, i) => (
            ...
            >
              <div className="flex items-baseline gap-0.5 justify-center mb-1">
                <span className="font-serif-num text-[32px] md:text-[40px] font-semibold text-white leading-none tracking-[-0.5px]">
                  {stat.useComma ? counts[i].toLocaleString() : counts[i]}
                </span>
                <span className="text-[16px] md:text-[18px] font-bold text-brand-orange-soft">
                  {stat.suffix}
                </span>
              </div>
